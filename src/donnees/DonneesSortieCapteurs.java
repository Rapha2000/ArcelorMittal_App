...
/** Cette classe représente les données qui sortent des capteurs **/
...
public class DonneesSortieCapteurs {
	...
	/** Cette méthode remplace la méthode toString() par défaut de la classe Object de Java.**/
	public String toString() {
		DecimalFormatSymbols mySymbol = new DecimalFormatSymbols();
		mySymbol.setDecimalSeparator('.');
	// Créer un nouvel objet DecimalFormat avec un format spécifique et définit le symbole de séparation décimale à utiliser.
		DecimalFormat df = new DecimalFormat("0.000", mySymbol);

	// Retourne une chaîne formatée représentant les champs de l'objet séparés par des virgules.
	// Chaque champ est formaté à l'aide de l'objet DecimalFormat.
